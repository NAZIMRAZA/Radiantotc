...
const AdminPanel: React.FC<AdminPanelProps> = ({ onUpdatePrice, currentPrice = 91.45 }) => {
  ...
  const handleDownloadCSV = async () => {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute('download', `kyc_records_${new Date().getTime()}.csv`);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <button
                  ...
                  className={`w-full py-4 rounded-xl font-black font-orbitron text-[10px] tracking-widest transition-all ${saveStatus === 'success' ? 'bg-green-500 text-black' : 'bg-cyan-500 text-black'
                    }`}
                >
        ...
        <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
          ...
          <div className="glass-card p-6 rounded-2xl border-orange-500/20 bg-orange-500/5 flex flex-col justify-between">
            <div>
              <p className="text-[9px] font-black text-orange-500 uppercase tracking-widest mb-1">KYC Database</p>
              <p className="text-2xl md:text-4xl font-black text-white font-orbitron">Secured</p>
            </div>
            <button
              onClick={handleDownloadCSV}
              disabled={isDownloading}
              className="mt-4 w-full bg-orange-500 text-black py-2 rounded-lg font-black font-orbitron tracking-widest text-[9px] uppercase hover:bg-orange-400 disabled:opacity-50"
            >
              {isDownloading ? 'EXTRACTING...' : 'DOWNLOAD .CSV EXCEL'}
            </button>
          </div>
          <div className="glass-card p-6 rounded-2xl border-white/5">
